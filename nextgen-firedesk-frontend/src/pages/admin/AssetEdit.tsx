...
export default function AssetEdit() {
  ...
  const handleUpdateAsset = async () => {
    ...
    setIsSaving(true);
    try {
      const { organizationId, ...updateData } = formData;
    ...
    <div className="flex-1 flex flex-col bg-background">
      <div className="flex-1 p-8">
          ...
          >
            {ASSET_TABS.map((tab) => (
              <TabsTrigger
                key={tab.value}
                value={tab.value}
                className="text-base py-3 px-5 data-[state=active]:border-b-2 data-[state=active]:border-orange-500 data-[state=active]:text-orange-600 rounded-none"
                style={{ marginLeft: 0 }}
              >
                {tab.label}
              </TabsTrigger>
        ...
        <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-border">
          ...
          >
            {isSaving ? "Updating..." : "Update Asset"}
          </Button>
        </div>
      </div>
    </div>
