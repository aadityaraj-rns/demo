...
const CreateGroupService = ({ fetchData }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, actions) => {
      ...
      if (response.status == 200) {
        ...
        actions.resetForm();
        showAlert({
          text: "Service-group created successfully",
          icon: "success",
        });
        fetchData();
      } else {
  ...
  return (
    <>
      <Button size="small" color="warning" variant="contained" onClick={toggle}>
