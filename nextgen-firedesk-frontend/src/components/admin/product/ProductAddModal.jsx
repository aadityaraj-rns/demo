...
const ProductAddModal = ({ onProductAdd }) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      testFrequency: "",
      productVariants: [
        { type: "", subType: [], description: "", image: null },
      ],
    },
    ...
    onSubmit: async (values, actions) => {
      ...
      try {
        ...
        if (res?.status == 200) {
          ...
          onProductAdd();
          toggle();
          showAlert({ text: "Product created successfully", icon: "success" });
        } else {
          toggle();
          showAlert({ text: res.data.message, icon: "error" });
        }
      } catch (err) {
        toggle();
